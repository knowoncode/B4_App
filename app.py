...
orgObj = OrgOperation()    # object for organiser
...
def user_logout():
    session.clear()   #it will destroy all activated session
    ...
    if 'org_email' in session:
        if request.method=='GET':
            ...
            if(validObj.checkDigit(mobile) or validObj.checkMobileLength(mobile)):
                ...
                return redirect(url_for('org_profile'))
            #-------- end validation------------------------
            orgObj.org_profile_update(name,mobile,address)
            flash("your profile is updated successfully!!")
            return redirect(url_for('org_profile'))
    else:
        flash("please login to access this page..")
...
def org_change_password():
    if 'org_email' in session:
        if request.method=='GET':
            return render_template('org_change_password.html')
        else:
            oldPassword=request.form['oldPassword']
            newPassword=request.form['newPassword']
            # -------------validation--------------------
...
def org_new_camp():
    if 'org_email' in session:
        if request.method=='GET':
            ...
            orgObj.org_new_camp(campName,contact,state,city,location,startDate,endDate,charges,descp)
...
def org_camp_detail():
    if 'org_email' in session:
        if request.method=="GET":
            ...
            flash("Camp Detail updated successfully!!")
            return redirect(url_for('org_camp_detail',campID=campID))
    else:
        flash("please login to access this page..")
        return redirect(url_for('org_login'))
...
def org_photo_delete():
    if 'org_email' in session:
        ...
        orgObj.org_photo_delete(campPhotoID)
...
def org_booking_info():
    if 'org_email' in session:
        record = orgObj.org_booking_info()
